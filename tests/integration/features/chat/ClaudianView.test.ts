...
function createTFile(path: string): TFile {
  const file = new (TFile as any)(path) as any;
  if (!file.stat) {
...
function getEditedFilesTracker(manager: FileContextManager): any {
  return (manager as any).editedFilesTracker;
}

function getMentionController(manager: FileContextManager): any {
  return (manager as any).mentionDropdown;
}
...
describe('FileContextManager - Edited Files Tracking', () => {
  ...
  beforeEach(() => {
    ...
    tracker = getEditedFilesTracker(fileContextManager);
  });
  ...
  describe('Tracking edited files from tool results', () => {
    it('should track file when Write tool completes successfully', async () => {
      ...
      expect(tracker.editedFilesThisSession.has(normalizedPath)).toBe(true);
    });
    ...
    it('should NOT track file when tool result has error', async () => {
      ...
      expect(tracker.editedFilesThisSession.has(normalizedPath)).toBe(false);
    });
    ...
    it('should NOT track files from Bash tool', async () => {
      ...
      expect(tracker.editedFilesThisSession.size).toBe(0);
    });
    ...
    it('should normalize absolute paths to vault-relative for tracking and dismissal', async () => {
      ...
      tracker.dismissEditedFile(rawPath);
      expect(tracker.editedFilesThisSession.has(normalizedPath)).toBe(false);
    });
  });
  ...
  describe('Clearing edited files', () => {
    it('should clear edited files on resetForNewConversation()', async () => {
      ...
      tracker.editedFilesThisSession.add('file1.md');
      tracker.editedFilesThisSession.add('file2.md');

      expect(tracker.editedFilesThisSession.size).toBe(2);
    ...
    it('should clear edited files on new conversation', async () => {
      tracker.editedFilesThisSession.add('old-file.md');
    ...
    it('should remove file from edited set when file is focused', async () => {
      ...
      expect(tracker.editedFilesThisSession.has(filePath)).toBe(true);
      expect(tracker.editedFileHashes.has(filePath)).toBe(true);

      // Simulate focusing on the file (via private method)
      tracker.dismissEditedFile(filePath);

      expect(tracker.editedFilesThisSession.has(filePath)).toBe(false);
      expect(tracker.editedFileHashes.has(filePath)).toBe(false);
    });
    ...
    it('should dismiss edited indicator when focusing file', async () => {
      ...
      expect(tracker.isFileEdited(filePath)).toBe(false);
    });
  });
});
...
describe('FileContextManager - File Chip Click Handlers', () => {
  ...
  describe('Opening files on chip click', () => {
    it('should open file in new tab when chip is clicked', async () => {
      ...
      await chipsView.callbacks.onOpenFile(filePath);
  ...
  describe('Edited class on chips', () => {
    it('should return true when file is in editedFilesThisSession', () => {
      ...
      const isEdited = tracker.isFileEdited(filePath);
...
describe('FileContextManager - Edited Files Section', () => {
  ...
  describe('Visibility logic', () => {
    it('should return non-attached edited files only', () => {
      ...
      tracker.editedFilesThisSession.add('edited1.md');
    ...
    it('should show edited files section when has non-attached edited files', () => {
      tracker.editedFilesThisSession.add('edited.md');
...
describe('FileContextManager - File Hash Tracking', () => {
  ...
  describe('markFileBeingEdited', () => {
    it('should mark file as being edited and capture original hash', async () => {
      ...
      expect(tracker.filesBeingEdited.has(filePath)).toBe(true);
      expect(tracker.editedFileHashes.has(filePath)).toBe(true);
      const hashState = tracker.editedFileHashes.get(filePath);
    ...
    it('should NOT mark file for non-edit tools', async () => {
      ...
      expect(tracker.filesBeingEdited.has(filePath)).toBe(false);
    });
  ...
  describe('trackEditedFile with hash tracking', () => {
    ...
    it('should refresh baseline on sequential edits', async () => {
      ...
      const firstState = tracker.editedFileHashes.get(filePath);
      ...
      const secondState = tracker.editedFileHashes.get(filePath);
  ...
  describe('cancelFileEdit', () => {
    it('should clean up state when permission is denied', () => {
      ...
      tracker.filesBeingEdited.add(filePath);
      tracker.editedFileHashes.set(filePath, {
        originalHash: 'hash1',
        postEditHash: '',
      });
  ...
  describe('File rename handling', () => {
    it('should update path when file is renamed', () => {
      ...
      tracker.editedFileHashes.set(oldPath, {
        originalHash: 'hash1',
        postEditHash: 'hash2',
      });
      ...
      expect(tracker.editedFilesThisSession.has(oldPath)).toBe(false);
      expect(tracker.editedFilesThisSession.has(newPath)).toBe(true);
      expect(tracker.editedFileHashes.has(oldPath)).toBe(false);
      expect(tracker.editedFileHashes.has(newPath)).toBe(true);
    });
  });
  ...
  describe('File revert detection', () => {
    it('should remove chip when content reverts to original', async () => {
      ...
      await tracker.handleFileModified(mockFile);
  ...
  describe('Session cleanup', () => {
    it('should clear all hash state on resetForNewConversation', () => {
      // Setup some state
      tracker.editedFilesThisSession.add('file1.md');
      tracker.editedFileHashes.set('file1.md', {
        originalHash: 'hash1',
        postEditHash: 'hash2',
      });
      tracker.filesBeingEdited.add('file2.md');

      fileContextManager.resetForNewConversation();

      expect(tracker.editedFilesThisSession.size).toBe(0);
      expect(tracker.editedFileHashes.size).toBe(0);
      expect(tracker.filesBeingEdited.size).toBe(0);
    });
...
describe('FileContextManager - Border indicator sync with @ mentions', () => {
  ...
  it('should remove border from attached file when file is deleted', () => {
    ...
    expect(tracker.isFileEdited(filePath)).toBe(true);
  ...
  it('should update border indicator path when attached file is renamed', () => {
    ...
    expect(tracker.isFileEdited(oldPath)).toBe(false);
    expect(tracker.isFileEdited(newPath)).toBe(true);
  });
});
...
describe('FileContextManager - @ Mention Dropdown', () => {
  ...
  beforeEach(() => {
    ...
    mentionController = getMentionController(fileContextManager);
  });
  ...
  describe('handleInputChange - whitespace detection', () => {
    ...
    it('should keep dropdown open when typing continuous text', () => {
      ...
      expect(mentionController.mentionStartIndex).toBe(0);
    });
    ...
    it('should trigger dropdown when @ follows whitespace', () => {
      ...
      expect(mentionController.mentionStartIndex).toBe(6);
    });
  });
  ...
  describe('handleMentionKeydown', () => {
    ...
    it('should handle Escape to close dropdown', () => {
      ...
      fileContextManager.handleInputChange();
      expect(fileContextManager.isMentionDropdownVisible()).toBe(true);
